...
class CustomerChangePasswordForm extends Component
{
    ...
    public function messages(): array
{
    ...
    public function save()
    {
        $this->validate();
        try{
            if (!Hash::check($this->form['current_password'], auth('customer')->user()->password)) {
                $this->errorFlash(__('Current Password does not match'));
                return redirect()->route('customer.profile.password-index');
            }
