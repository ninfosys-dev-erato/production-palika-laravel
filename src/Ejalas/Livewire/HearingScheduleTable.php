...
class HearingScheduleTable extends DataTableComponent
{
    ...
    public function columns(): array
    {
        ...
        if (!$this->report && (can('hearing_schedules edit') || can('hearing_schedules delete'))) {
            $actionsColumn = Column::make(__('ejalas::ejalas.actions'))->label(function ($row, Column $column) {
                $buttons = '';

                if (can('hearing_schedules edit')) {
                    $edit = '<button class="btn btn-primary btn-sm" wire:click="edit(' . $row->id . ')" ><i class="bx bx-edit"></i></button>&nbsp;';
                    $buttons .= $edit;
                }

                if (can('hearing_schedules delete')) {
                ...
                if (can('hearing_schedules print')) {
