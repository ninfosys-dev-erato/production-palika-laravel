<div>
            ...
            <div class="d-flex justify-content-between align-items-center mx-4 mb-0">
                <div class="divider divider-primary text-start text-primary fw-bold flex-grow-1 mb-0">
                    <div class="divider-text fs-5">
                        {{ __('ejalas::ejalas.dispute_deadline_report') }}
                    </div>
                </div>
    ...
    <div class="overflow-x-auto mx-auto">
        @if ($disputeDeadlines && $disputeDeadlines->count())
                    ...
                    <table class="bordered-table">
                        <thead>
                            <tr>
                                ...
                                <th>{{ __('ejalas::ejalas.deadline_date') }}</th>
                                <th>{{ __('ejalas::ejalas.deadline_extension_period') }}</th>
                                <th>{{ __('ejalas::ejalas.registered_by') }}</th>

                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($disputeDeadlines as $deadline)
                                <tr>
                                    <td>{{ $deadline->complaintRegistration?->reg_no ?? '' }}</td>
                                    <td>{{ $deadline->complaintRegistration?->disputeMatter?->title ?? '' }}</td>
                                    <td>
                                        {{ $deadline->complaintRegistration?->complainers->pluck('name')->implode(', ') ?? '' }}
                                    </td>

                                    <td>
                                        {{ $deadline->complaintRegistration?->defenders->pluck('name')->implode(', ') ?? '' }}
                                    </td>

                                    <td>{{ $deadline->deadline_set_date ?? '' }}</td>
                                    <td>{{ replaceNumbers($deadline->deadline_extension_period, true) ?? '' }}</td>
                                    <td>{{ $deadline->judicialEmployee?->name ?? '' }}</td>
