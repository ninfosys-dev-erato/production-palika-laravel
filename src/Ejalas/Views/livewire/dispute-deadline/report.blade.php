<div>
    ...
    <div class="card-header d-flex justify-content-between align-items-center">
        <h4 class="text-primary mb-0">{{ __('ejalas::ejalas.dispute_deadline_report') }}</h4>
                    ...
                    <table class="table table-border">
                        <thead>
                            <tr>
                                ...
                                <th>{{ __('ejalas::ejalas.deadline_type') }}</th>
                        ...
                        <tbody>
                            @foreach ($disputeDeadlines as $deadline)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td>{{ implode(', ', $deadline->complainers ?? []) }}</td>
                                    <td>{{ implode(', ', $deadline->defenders ?? []) }}</td>
                                    <td>{{ $deadline->complaintRegistration?->disputeMatter?->title ?? '' }}</td>
                                    <td>{{ $deadline->deadline_date_bs ?? '' }}</td>
                                    <td>{{ $deadline->deadline_type ?? '' }}</td>
                                    <td>{{ $deadline->is_completed ? __('ejalas::ejalas.completed') : __('ejalas::ejalas.pending') }}
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
