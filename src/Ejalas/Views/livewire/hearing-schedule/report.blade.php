<div>
    ...
    <div class="card-header d-flex justify-content-between align-items-center">
        <h4 class="text-primary mb-0">{{ __('ejalas::ejalas.hearing_schedule_report') }}</h4>
                        ...
                        <tbody>
                            @foreach ($hearingSchedules as $hearingSchedule)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td>{{ implode(', ', $hearingSchedule->complainers ?? []) }}

                                    </td>
                                    <td>{{ implode(', ', $hearingSchedule->defenders ?? []) }}
                                    </td>
                                    <td>{{ $hearingSchedule->complaintRegistration?->disputeMatter?->title ?? '' }}
                                    </td>
                                    <td>{{ $hearingSchedule->hearing_date_bs ?? '' }}</td>
                                    <td>{{ $hearingSchedule->hearing_time ?? '' }}</td>
                                    <td>{{ $hearingSchedule->reconciliationCenter?->title ?? '' }}</td>
                                    <td>{{ $hearingSchedule->status ? __('ejalas::ejalas.completed') : __('ejalas::ejalas.pending') }}
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
