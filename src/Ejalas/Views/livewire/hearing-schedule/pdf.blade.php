...
<body>
    ...
    <table>
        <thead>
            <tr>
                <th style="width: 30px;">तारेख पर्चा संख्या</th>
                <th style="width: 30px;">तारेख पर्चा जारी मिति</th>
                <th style="width: 30px;">मुद्दा नं</th>
                <th style="width: 30px;">तारेख नं</th>
                <th style="width: 100px;">निवेदक</th>
                <th style="width: 100px;">विपक्षी</th>
                <th style="width: 50px;">विवादको क्षेत्र</th>
                <th style="width: 150px;">विवादको विषय</th>
        ...
        <tbody>
            @foreach ($reports as $report)
                <tr class="hover:bg-gray-50">
                    <td>{{ $report->hearing_paper_no }}</td>
                    <td>{{ $report->hearing_date_bs }}</td>
                    <td>{{ $report->complaintRegistration->reg_no }}</td>
                    <td>{{ $report->reference_no }}</td>
                    <td>{{ implode(', ', $report->complainers ?? []) }}</td>
                    <td>{{ implode(', ', $report->defenders ?? []) }}</td>
                    <td>{{ $report->complaintRegistration->disputeMatter?->disputeArea->title ?? '' }}</td>
                    <td>{{ $report->complaintRegistration->disputeMatter?->title ?? '' }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
