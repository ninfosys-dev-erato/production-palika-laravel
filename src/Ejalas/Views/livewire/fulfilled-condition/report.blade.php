<div>
    <nav aria-label="breadcrumb" class="d-flex justify-content-end">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="#">{{ __('ejalas::ejalas.fulfilled_condition') }}</a>
            </li>
            <li class="breadcrumb-item active" aria-current="page">{{ __('ejalas::ejalas.list') }}</li>
        </ol>
    </nav>
    <div class="card-header d-flex justify-content-between align-items-center">
        <h4 class="text-primary mb-0">{{ __('ejalas::ejalas.fulfilled_condition_report') }}</h4>
    ...
    <div class="overflow-x-auto mx-auto">
        @if ($fulfilledConditions && $fulfilledConditions->count())
                    ...
                    <table class="table table-border">
                        <thead>
                            <tr>
                                ...
                                <th>{{ __('ejalas::ejalas.condition_title') }}</th>
                                <th>{{ __('ejalas::ejalas.fulfillment_date') }}</th>
                                <th>{{ __('ejalas::ejalas.status') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($fulfilledConditions as $condition)
                                <tr class="hover:bg-gray-50">
                                    <td>{{ $condition->complaintRegistration?->reg_no ?? '' }}</td>
                                    <td>{{ implode(', ', $condition->complainers ?? []) }}</td>
                                    <td>{{ implode(', ', $condition->defenders ?? []) }}</td>
                                    <td>{{ $condition->complaintRegistration?->disputeMatter?->title ?? '' }}</td>
                                    <td>{{ $condition->condition_title ?? '' }}</td>
                                    <td>{{ $condition->fulfillment_date_bs ?? '' }}</td>
                                    <td>{{ $condition->is_fulfilled ? __('ejalas::ejalas.fulfilled') : __('ejalas::ejalas.pending') }}
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
