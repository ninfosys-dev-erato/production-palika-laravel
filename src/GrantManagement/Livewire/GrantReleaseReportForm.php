...
use Src\GrantManagement\Exports\GrantReleaseExports;
use Src\GrantManagement\Models\GrantRelease;
use Illuminate\Support\Facades\Auth;
use App\Facades\GlobalFacade;
use App\Facades\PdfFacade;

class GrantReleaseReportForm extends Component
{
    ...
    public function rules(): array
    {
        return [
            'start_date' => 'required',
            'end_date' => 'required',
            'selectedFiscalYear' => 'required',
            'selctedGrantType' => 'required',
            'selectedBranchType' => 'required',
            'selctedGrantGivingOrg' => 'required',
        ];
    }
    ...
    public function messages(): array
    {
        return [
            ...
            'selectedFiscalYear.required' => 'Please select a fiscal year.',
            'selctedGrantType.required' => 'Please select a grant type.',
            'selectedBranchType.required' => 'Please select a branch type.',
            'selctedGrantGivingOrg.required' => 'Please select a grant giving organization.',
        ];
    }
    ...
    {
        $this->action = $action;
        $this->verifiedUsers = Customer::whereNotNull('kyc_verified_at')
    ...
    public function showRelativeData()
    {
        ...
        $query = GrantRelease::query();

        $query->whereBetween('created_at', [$startDateAd, $endDateAd]);

        if (!empty($this->selectedUsers)) {
            $query->whereIn('customer_id', (array) $this->selectedUsers);
        }

        if(!empty($this->selectedGranteeType)){
            $query->whereIn('grantee_type', (array) $this->selectedGranteeType);
        }

        $query->with(['grantProgram']);
    ...
    {
        $this->reset(['start_date', 'end_date', 'selectedUsers', 'filtered_datas']);
    ...
    public function export()
    {
        ...
        $exportFilePath = 'grant-release-report.xlsx';
        return Excel::download(new GrantReleaseExports($this->filtered_datas), $exportFilePath);
    }
    ...
    public function downloadPdf()
    {
        ...
        $html = view('GrantManagement::pdf.grantRelease', compact('nepaliDate', 'palika_name', 'palika_logo', 'palika_campaign_logo', 'address', 'palika_ward', 'reports', 'startDate', 'endDate'))->render();
    ...
    {
        return view("GrantManagement::livewire.grant-release-report-form");
    }
}
