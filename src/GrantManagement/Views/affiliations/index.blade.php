<x-layout.app header="{{ __('grantmanagement::grantmanagement.affiliation_list') }}">
    <nav aria-label="breadcrumb" class="d-flex justify-content-end">
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}"><i class="bx bx-home-alt"></i></a>
            <li class="breadcrumb-item"><a href="#">{{ __('grantmanagement::grantmanagement.affiliation') }}</a>
            </li>
                ...
                <div class="card-header  d-flex justify-content-between">
                    <div class="d-flex justify-content-between card-header">
                        <h5 class="text-primary fw-bold mb-0">
                            {{ __('grantmanagement::grantmanagement.affiliation_list') }}
                        </h5>
                    </div>
                    <div>
                        @perm('gms_settings create')
                            ...
                            <button class="btn btn-info" data-bs-toggle="modal" onclick="resetForm()"
                                ...
                                <i class="bx bx-plus"></i> {{ __('grantmanagement::grantmanagement.add_affiliation') }}
                            </button>
                        @endperm
                    </div>
                </div>
                <div class="card-body">
                    <livewire:grant_management.affiliation_table theme="bootstrap-4" />
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="modal-content ">
                <div class="modal-header">
                    <h5 class="modal-title" id="AddEnterpriseTypeListLabel">
                        {{ __('grantmanagement::grantmanagement.affiliation_type') }}
                    </h5>
                    <button type="button" class="btn-close" onclick="resetForm()" data-bs-dismiss="modal"
                        aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <livewire:grant_management.affiliation_form :action="App\Enums\Action::CREATE" />
                </div>
            </div>
        </div>
    </div>
