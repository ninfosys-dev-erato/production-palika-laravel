...
class PlanNearDeadline extends Component
{
    ...
    {
        return view("Yojana::livewire.reports.plan-near-deadline");
    }
    ...
    public function mount()
    {
        ...
        $this->plans = Plan::whereNull('deleted_at')
            ->with(['ward','agreement','implementationMethod','implementationAgency.consumerCommittee', 'implementationAgency.organization', 'implementationAgency.application', 'StartFiscalYear'])
            ->get()
            ->filter(function ($plan) {
                $bsDate = $plan->agreement?->plan_completion_date;
                $adDate = $bsDate ? $this->bsToAd($bsDate) : null;

                return $adDate &&
                    Carbon::parse($adDate)->between(
                        Carbon::now(),
    ...
    public function search()
    {
        ...
        $plans = Plan::whereNull('deleted_at')
            ...
            ->when($this->projectName, fn($q) =>
            $q->where('project_name', 'like', '%' . $this->projectName . '%')
            )
            ->when($this->projectLocation, fn($q) =>
            $q->where('location', 'like', '%' . $this->projectLocation . '%')
            )
            ->when($this->selectedPlanArea, fn($q) =>
            $q->where('area_id', $this->selectedPlanArea)
            )
            ->when($this->selectedExpenseHead, fn($q) =>
            $q->whereHas('budgetSources.expenseHead', function ($q) {
                $q->where('id', $this->selectedExpenseHead);
            }))
            ->when($this->selectedWard, fn($q) =>
            $q->where('ward_id', $this->selectedWard)
            )
            ->get();

        // Now filter in PHP using BS-to-AD conversion
        $this->plans = $plans->filter(function ($plan) {
            $bsDate = $plan->agreement?->plan_completion_date;

            if (!$bsDate) return false;

            $adDate = $this->bsToAd($bsDate); // assumes YYYY-MM-DD output

            return $adDate && Carbon::parse($adDate)->between(
                    Carbon::now(),
                    Carbon::now()->addDays(15)
                );
    ...
    public function downloadPdf()
    {
        try {
            ...
            $html = view('Yojana::livewire.reports.pdf.pdf-overdue-plan', compact('nepaliDate',  'plans', 'header'))->render();
