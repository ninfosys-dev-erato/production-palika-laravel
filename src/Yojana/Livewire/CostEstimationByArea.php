...
class CostEstimationByArea extends Component
{
    ...
    {
        return view("Yojana::livewire.reports.cost-estimation-by-area");
    }
    ...
    public function mount()
    {
        ...
        $this->query = PlanArea::whereNull('deleted_at')
            ->whereHas('plans')
    ...
    public function search()
    {
        ...
        $planAreas = PlanArea::whereNull('deleted_at')
            ->whereHas('plans')
            ->with('plans.budgetSources', 'plans.costEstimation', 'plans.payments');

        if ($this->selectedPlanArea) {
            $planAreas->where('id', $this->selectedPlanArea);
        }

        $this->query = $planAreas->get();
    }
    ...
    public function downloadPdf()
    {
        try {
            ...
            $html = view('Yojana::livewire.reports.pdf.pdf-cost-estimation-by-area', compact('nepaliDate',  'startDate', 'endDate', 'query', 'header'))->render();
