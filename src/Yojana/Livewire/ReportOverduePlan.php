...
class ReportOverduePlan extends Component
{
    ...
    {
        return view("Yojana::livewire.reports.overdue-plan-report");
    }
    ...
    public function mount()
    {
        ...
        $this->plans = Plan::whereNull('deleted_at')
            ->with('agreement', 'StartFiscalYear', 'implementationMethod','implementationAgency.consumerCommittee', 'implementationAgency.organization', 'implementationAgency.application',)
            ->whereHas('agreement', function ($q) {
                $q->where('plan_completion_date', '<=', Carbon::now()->toDateString());
            })
            ->get();
