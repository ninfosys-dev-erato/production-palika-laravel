...
class TaxDeductionReport extends Component
{
    ...
    {
        return [
            'selectedConfiguration' => 'nullable',
        ];
    }
    ...
    {
        return view("Yojana::livewire.reports.tax-deduction-report");
    }

    public function mount()
    {
        $this->configurations = Configuration::whereNull('deleted_at')
            ->pluck('title', 'id');

        $this->query = Configuration::whereNull('deleted_at')
            ->whereHas('taxDeductions')
            ->with('taxDeductions.payment.plan')
            ->get();
    ...
    public function search()
    {
        ...
        $configurations = Configuration::whereNull('deleted_at')
            ->whereHas('taxDeductions')
            ->with('taxDeductions.payment.plan');

        if ($this->selectedConfiguration) {
            $configurations->where('id', $this->selectedConfiguration);
        }

        $this->query = $configurations->get();
    }
    ...
    public function downloadPdf()
    {
        try {
            $query =  $this->query;
            $query->load('taxDeductions.payment.plan');
            ...
            $html = view('Yojana::livewire.reports.pdf.pdf-tax-deduction', compact('nepaliDate',  'startDate', 'endDate', 'query', 'header'))->render();
