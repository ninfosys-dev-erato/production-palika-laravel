...
class AgreedPlansTable extends DataTableComponent
{
    ...
    public function searchData($area_id, $budgetHead_id, $expenseHead_id, $planType, $projectGroup_id, $implementationLevel_id)
    {
        ...
        $this->budgetHead_id = $budgetHead_id ?? null;
        $this->expenseHead_id = $expenseHead_id ?? null;
        $this->planType = $planType ?? null;
        $this->projectGroup_id = $projectGroup_id ?? null;
        $this->implementationLevel_id = $implementationLevel_id ?? null;
    }
    ...
    public function extendDate($id)
    {
        ...
        $workOrder = $service->workOrderLetter(LetterTypes::TimeExtension, Plan::find($id));
        if($workOrder?->id){
            return redirect()->away(route('admin.plans.work_orders.preview',['id'=>$workOrder->id]));
        }else{
            $this->errorFlash(__('yojana::yojana.letter_sample_not_found'));
        }
    }
}
