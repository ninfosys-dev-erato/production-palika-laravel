...
class CostEstimationByBudgetSource extends Component
{
    ...
    public $selectedBudgetSource;
    public $query;

    public function rules(): array
    {
        return [
            'selectedBudgetSource' => 'nullable',
        ];
    }
    ...
    {
        return view("Yojana::livewire.reports.cost-estimation-by-budget-source");
    }

    public function mount()
    {
        $this->budgetSources = BudgetSource::whereNull('deleted_at')
            ->pluck('title', 'id');

        $this->query = BudgetSource::whereNull('deleted_at')
            ->whereHas('plans')
            ->with('plans.costEstimation', 'plans.payments')
            ->get();
        //        dd($this->query);
    ...
    public function search()
    {
        ...
        $budgetSources = BudgetSource::whereNull('deleted_at')
            ->whereHas('plans')
            ->with('plans.costEstimation', 'plans.payments');

        if ($this->selectedBudgetSource) {
            $budgetSources->where('id', $this->selectedBudgetSource);
        }

        $this->query = $budgetSources->get();
    }
    ...
    public function downloadPdf()
    {
        try {
            ...
            $html = view('Yojana::livewire.reports.pdf.pdf-cost-estimation-budgetSource', compact('nepaliDate',  'startDate', 'endDate', 'query', 'header'))->render();
