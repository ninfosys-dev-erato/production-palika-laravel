...
use Src\Yojana\Exports\FuelRatesExport;
use Src\Yojana\Models\FuelRate;
use Src\Yojana\Service\FuelRateAdminService;

class uelRateTable extends DataTableComponent
{
    use SessionFlash,IsSearchable;
    protected $model = FuelRate::class;
...
Column::make("Has Included Vat", "has_included_vat") ->sortable()->searchable()->collapseOnTablet(),
     ];
        if (can('fuel_rates edit') || can('fuel_rates delete')) {
            $actionsColumn = Column::make('Actions')->label(function ($row, Column $column) {
                $buttons = '<div class="btn-group" role="group" >';

                if (can('fuel_rates edit')) {
                    $edit = '<button class="btn btn-primary btn-sm" wire:click="edit(' . $row->id . ')" ><i class="fa fa-edit"></i></button>&nbsp;';
                    $buttons .= $edit;
                }

                if (can('fuel_rates delete')) {
    ...
    public function edit($id)
    {
        ...
        return redirect()->route('admin.fuel_rates.edit',['id'=>$id]);
    }
    public function delete($id)
    {
        ...
        $service->delete(FuelRate::findOrFail($id));
        $this->successFlash("Fuel Rate Deleted Successfully");
    }
    ...
    public function exportSelected(){
        ...
        $this->clearSelected();
        return Excel::download(new FuelRatesExport($records), 'fuel_rates.xlsx');
    }
}
