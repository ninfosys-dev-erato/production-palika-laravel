<x-layout.app header="{{ __('yojana::yojana.project_group_list') }}">
    <nav aria-label="breadcrumb" class="d-flex justify-content-end">
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}"><i class="bx bx-home-alt"></i></a>
            <li class="breadcrumb-item"><a
                    href="{{ route('admin.plan.index') }}">{{ __('yojana::yojana.plan_management') }}</a>
            <li class="breadcrumb-item"><a href="#">{{ __('yojana::yojana.project_group') }}</a>
            </li>
            ...
            <div class="card">
                <div class="card-header  d-flex justify-content-between">
                    <div class="d-flex justify-content-between card-header">
                        <h5 class="text-primary fw-bold mb-0">{{ __('yojana::yojana.project_group_list') }}</h5>
                    </div>
                    <div>
                        @perm('project_groups create')
                            <a data-bs-toggle="modal" data-bs-target="#indexModal" class="btn btn-info"
                                onclick="resetForm()"><i class="bx bx-plus"></i>
                                {{ __('yojana::yojana.add_project_group') }}</a>
                        @endperm
                    </div>
                </div>
                <div class="card-body">
                    <livewire:yojana.project_group_table theme="bootstrap-4" />
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="ModalLabel">
                        {{ __('yojana::yojana.manage_project_group') }}
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" onclick="resetForm()"
                        aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <livewire:yojana.project_group_form :action="App\Enums\Action::CREATE" />
                </div>
            </div>
        </div>
    </div>
