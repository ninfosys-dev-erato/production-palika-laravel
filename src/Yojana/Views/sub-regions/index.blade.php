<x-layout.app header="{{ __('yojana::yojana.sub_region_list') }}">
    <nav aria-label="breadcrumb" class="d-flex justify-content-end">
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}"><i class="bx bx-home-alt"></i></a>
            <li class="breadcrumb-item"><a
                    href="{{ route('admin.plan.index') }}">{{ __('yojana::yojana.plan_management') }}</a>
            <li class="breadcrumb-item"><a href="#">{{ __('yojana::yojana.sub_region') }}</a>
            </li>
            ...
            <div class="card">
                <div class="card-header  d-flex justify-content-between">
                    <div class="d-flex justify-content-between card-header">
                        <h5 class="text-primary fw-bold mb-0">{{ __('yojana::yojana.sub_region_list') }}</h5>
                    </div>
                    <div>
                        @perm('sub_regions create')
                            <button data-bs-toggle="modal" data-bs-target="#indexModal" class="btn btn-info"
                                onclick="resetForm()"><i class="bx bx-plus"></i>
                                {{ __('yojana::yojana.add_sub_region') }}</button>
                        @endperm
                    </div>
                </div>
                <div class="card-body">
                    <livewire:yojana.sub_region_table theme="bootstrap-4" />
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="ModalLabel">
                        {{ __('yojana::yojana.manage_sub_region') }}
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" onclick="resetForm()"
                        aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <livewire:yojana.sub_region_form :action="App\Enums\Action::CREATE" />
                </div>
            </div>
        </div>
    </div>
