<x-layout.app header="{{ __('yojana::yojana.budget_transfer_list') }}">
    <nav aria-label="breadcrumb" class="d-flex justify-content-end">
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}"><i class="bx bx-home-alt"></i></a>
            <li class="breadcrumb-item"><a
                    href="{{ route('admin.plan.index') }}">{{ __('yojana::yojana.plan_management') }}</a>
            <li class="breadcrumb-item"><a href="#">{{ __('yojana::yojana.budget_transfer') }}</a>
            </li>
                ...
                <div class="card-header  d-flex justify-content-between">
                    ...
                    <div>
                        @perm('plan create')
                            ...
                            {{--                                class="btn btn-info"><i class="bx bx-plus"></i> {{ __('yojana::yojana.add_budget_transfer') }}</button> --}}
                        @endperm
                    </div>
                </div>
                <div class="card-body">
                    <livewire:yojana.budget_transfer_table theme="bootstrap-4" />
                </div>
            </div>
        </div>
    </div>
    ...
    {{-- <livewire:yojana.modal_form :component="'yojana.budget_transfer_form'" :action="App\Enums\Action::CREATE" /> --}}
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="ModalLabel">
                        {{ __('yojana::yojana.manage_budget_transfer') }}
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" onclick="resetForm()"
                        aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <livewire:yojana.budget_transfer_form :action="App\Enums\Action::CREATE" />
                </div>
            </div>
        </div>
    </div>
