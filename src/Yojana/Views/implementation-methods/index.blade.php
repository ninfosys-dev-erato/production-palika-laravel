<x-layout.app header="{{ __('yojana::yojana.implementation_method_list') }}">
    <nav aria-label="breadcrumb" class="d-flex justify-content-end">
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}"><i class="bx bx-home-alt"></i></a>
            <li class="breadcrumb-item"><a
                    href="{{ route('admin.plan.index') }}">{{ __('yojana::yojana.plan_management') }}</a>
            <li class="breadcrumb-item"><a href="#">{{ __('yojana::yojana.implementation_method') }}</a>
            </li>
            ...
            <div class="card">
                <div class="card-header  d-flex justify-content-between">
                    <div class="d-flex justify-content-between card-header">
                        <h5 class="text-primary fw-bold mb-0">{{ __('yojana::yojana.implementation_method_list') }}</h5>
                    </div>
                    <div>
                        @perm('implementation_methods create')
                            <a data-bs-target="#indexModal" data-bs-toggle="modal" onclick="resetForm()"
                                class="btn btn-info"><i class="bx bx-plus"></i>
                                {{ __('yojana::yojana.add_implementation_method') }}</a>
                        @endperm
                    </div>
                </div>
                <div class="card-body">
                    <livewire:yojana.implementation_method_table theme="bootstrap-4" />
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="ModalLabel">
                        {{ __('yojana::yojana.manage_implementation_method') }}
                    </h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" onclick="resetForm()"
                        aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <livewire:yojana.implementation_method_form :action="App\Enums\Action::CREATE" />
                </div>
            </div>
        </div>
    </div>
