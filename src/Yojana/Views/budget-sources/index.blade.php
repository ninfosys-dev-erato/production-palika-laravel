<x-layout.app header="{{ __('yojana::yojana.budget_source_list') }}">
    <nav aria-label="breadcrumb" class="d-flex justify-content-end">
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}"><i class="bx bx-home-alt"></i></a>
            <li class="breadcrumb-item"><a
                    href="{{ route('admin.plan.index') }}">{{ __('yojana::yojana.plan_management') }}</a>
            <li class="breadcrumb-item"><a href="#">{{ __('yojana::yojana.budget_source') }}</a>
            </li>
            ...
            <div class="card">
                <div class="card-header  d-flex justify-content-between">
                    <div class="d-flex justify-content-between card-header">
                        <h5 class="text-primary fw-bold mb-0">{{ __('yojana::yojana.budget_source_list') }}</h5>
                    </div>
                    <div>
                        @perm('budget_sources create')
                            <button data-bs-target="#indexModal" data-bs-toggle="modal" onclick="resetForm()"
                                class="btn btn-info"><i class="bx bx-plus"></i>
                                {{ __('yojana::yojana.add_budget_source') }}</button>
                        @endperm
                    </div>
                </div>
                <div class="card-body">
                    <livewire:yojana.budget_source_table theme="bootstrap-4" />
                </div>
            </div>
        </div>
    </div>
    ...
    {{-- <livewire:yojana.modal_form :component="'yojana.budget_source_form'" :action="App\Enums\Action::CREATE" /> --}}
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <livewire:yojana.budget_source_form :action="App\Enums\Action::CREATE" />
                </div>
            </div>
        </div>
    </div>
