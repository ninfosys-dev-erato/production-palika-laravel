...
<body>
    ...
    <div class="text-center">
        <h4>क्षेत्र अनुसार वडा-स्तरिय योजनाहरुको प्रतिवेदन</h4>
    </div>
        ...
        <tbody>
            @foreach ($query as $index => $row)
                <tr>
                    ...
                    <td>रु
                        {{ replaceNumbers(
                            $row->plans->filter(fn($plan) => $plan->agreement && $plan->agreement->agreementCost)->sum(fn($plan) => $plan->agreement->agreementCost->total_with_vat ?? 0),
                            true,
                        ) }}
                    </td>
