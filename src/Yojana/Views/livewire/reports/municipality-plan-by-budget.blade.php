<div>
    <div class="card-header d-flex justify-content-between align-items-center">
        <h4 class="text-primary fw-bold mb-0">{{ __('yojana::yojana.municipality_level_plans_according_to_budget') }}</h4>
                ...
                <div class="container-fluid">
                    <div class="row g-3">
                        ...
                        <div class="col-md-4 col-12">
                            <label class="form-label">{{ __('yojana::yojana.budget') }}</label>
                            <select class="form-select" wire:model="selectedBudgetDetail">
                                <option value="">{{ __('yojana::yojana.all') }}</option>
                                @foreach ($budgetDetails as $id => $name)
            ...
            <table class="table table-bordered table-striped">
                ...
                @foreach ($query as $index => $row)
                    <tr>
                        ...
                        <td>
                        @php
                                        $allPlans = collect($this->query)->pluck('plans')->flatten();
                    ...
                    @foreach ($grouped as $method => $plans)
                        ...
                        <td>{{ replaceNumbers($row->plans
                                ->filter(fn($plan) => $plan->nature == \Src\Yojana\Enums\Natures::Construction)
                                ->count(),true) }}
                        </td>
                        <td>{{ replaceNumbers($row->plans
