<div class="d-flex h-100 w-100 pt-3">
        ...
        <div class="w-100 d-flex justify-content-between align-items-center rounded-2 px-2 py-1 text-light mt-2"
            ...
            <div class="d-flex align-items-center">
                <div class="rounded-circle bg-light d-flex justify-content-center align-items-center me-2"
                    ...
                    <span class="text-primary" style="font-size: 0.8rem">५
                    </span>
                </div>
                <p class="m-0" style="font-size: 0.8rem">कर्मचारीहरु</p>
            </div>
        </div>
    </div>

    <div class="h-100 w-100 pb-3 d-flex flex-column align-items-center ms-3" style="width: 75%">
        <div class="w-75 h-100 my-4 mx-3">
            <div class="d-flex justify-content-center mb-4  rounded-1" style="background-color: var(--primary)">
                <p class="text-base fw-medium text-white py-1 m-0">कर्मचारीहरुको विवरण</p>
            </div>
                ...
                <div class="representative-grid ms-2" style="overflow: hidden" wire:poll.5s>
                    @foreach ($employees as $index => $emp)
                        <div class="representative-profile m-2" data-index="{{ $index }}"
                            wire:key="representative-{{ $emp->id }}">
                            <div class="card rounded-1 shadow-sm border-0 overflow-hidden">
                                <div class="w-100 d-flex justify-content-center" style="height: 4rem">
                                    <img src="{{ customAsset(config('src.Employees.employee.photo_path'), $emp->photo) }}"
                                        class="h-100" style="width: fit-content;" alt="{{ $emp->name }}">
                                </div>
                                <div class="card-body text-center">
                                    <p class="mb-0 text-dark" style="font-size: 0.7rem;">{{ $emp->name }}</p>
                                    <p class="mb-0 text-muted" style="font-size: 0.6rem;">
                                        {{$emp->designation->title ?? 'No Designation'  }}
                                    </p> <!-- Correct designation -->
                                    <p class="mb-0 text-muted" style="font-size: 0.5rem;">{{ $emp->phone }}</p>
                                    <!-- Correct phone -->
                                    <p class="mb-0 text-muted" style="font-size: 0.4rem;">{{ $emp->email }}</p>
                                    <!-- Correct email -->
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        function updateButtonStates() {
            ...
            const totalPages = Math.ceil({{ count($employees) }} / itemsPerPage);
