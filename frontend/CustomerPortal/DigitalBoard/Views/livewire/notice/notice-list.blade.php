<div
    class="flex flex-col sm:max-w-[800px] lg:max-w-[900px] xl:max-w-[1000px] 2xl:max-w-[1400px] sm:mx-auto mx-2 lg:flex-row gap-0 sm:gap-8 font-inter my-0 xl:my-8 md:justify-between">
    <div
        ...
        <div class="relative mb-1">
            ...
            @if ($isDropdownVisible)
                <div class="absolute top-10 left-0 bg-white shadow-md rounded-md w-full z-10 max-h-64 overflow-y-auto">
                    <ul>
                        <li class="px-4 py-2 hover:bg-gray-100 cursor-pointer" wire:click="showAllNotices">
                            {{ __('All Wards') }}
                        </li>

                        @foreach ($wards as $key => $ward)
                            <li class="px-4 py-2 hover:bg-gray-100 cursor-pointer"
                                wire:click="filterWard({{ $key + 1 }})">
                                {{ 'Ward' }} - {{ $ward }}
                            </li>
                        @endforeach
                    </ul>
    ...
    <div class="w-full mx-auto md:w-4/6">
        <div class="flex flex-col">
            <h2 class="text-base xl:text-2xl mx-3 md:mx-0 font-semibold mb-2">{{ __('Notices') }}</h2>
            <div class="md:w-full flex flex-col md:flex-row mx-3 md:mx-0 md:justify-between gap-2 md:gap-6">
                ...
                <button wire:click="searchNotices"
        ...
        <div
            class="bg-[#EEF1F8] border border-background md:w-full mx-3 md:mx-0 mt-6 rounded-lg px-4 py-4 md:px-16 md:py-7 mb-3 md:mb-auto">
            <div class="flex justify-between px-2 md:px-7 mb-2 md:mb-5">
                <div class="flex gap-2 items-center">
                    <p class="text-[#4E4E4E] text-sm md:font-bold">{{ __('Title') }}</p>
                    <img src="{{ asset('digitalBoard/icons/up-arrow.png') }}" alt="arrow up"
                        class="w-2 md:h-3 md:w-3 h-2">
                </div>
                <div class="flex gap-2 items-center">
                    <p class="text-[#4E4E4E] text-sm md:font-bold">{{ __('Issue Date') }}</p>
                    <img src="{{ asset('digitalBoard/icons/up-arrow.png') }}" alt="arrow up"
                        class="w-2 md:h-3 md:w-3 h-2">
                </div>
            </div>
            <div class="w-full min-h-[50vh] flex flex-col gap-3 overflow-hidden">
                @foreach ($notices as $notice)
                    <a href="{{ route('digital-board.notice.showDetail', ['id' => $notice->id]) }}">
                        <div class="bg-white rounded-xs px-5 flex justify-between py-1.5 cursor-pointer">
                            <p class="text-[11px] md:text-[15px] text-[#5B5B5B]">{{ $notice->title }}</p>
                            <p class="text-[9px] md:text-[12px] text-[#757575] font-semibold">{{ $notice->date }}</p>
                        </div>
                    </a>
                @endforeach
        ...
        <div class="flex items-center justify-between mt-4 mx-4">
            <div class="relative">
                ...
                <ul id="dropdownMenu"
                    class="absolute z-10 hidden w-20 mt-1 bg-white border border-gray-300 rounded shadow-md">
                    <li>
                    ...
                    <li>
                        <button wire:click="$set('perPage', 100)"
                            class="block w-full px-4 py-2 text-left text-sm hover:bg-gray-100">
                            50
                        </button>
                    </li>
