...
class BuildingRegistrationApplyStepForm extends Component
{
    ...
    public function save($formId, $data = null)
    {
        try{
            ...
            return redirect()->route('customer.ebps.building-registrations.step', ['id'=>$this->mapApply->id]);

        }catch (\Throwable $e){
            logger($e->getMessage());
            $this->errorFlash(((__('ebps::ebps.something_went_wrong_while-saving.') . $e->getMessage())));
        }
    }

    public function render()
    {
        return view("CustomerPortal.Ebps::livewire.print");
    }
