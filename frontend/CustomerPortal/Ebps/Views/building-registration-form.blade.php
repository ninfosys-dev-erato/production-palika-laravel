        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    ...
                    @endif
                    <div>
                        <a href="{{ route('customer.ebps.building-registrations.index') }}" class="btn btn-info">
                            <i class="bx bx-list-ol"></i>{{ __('ebps::ebps.building_registration_list') }}
                        </a>
                    </div>
                </div>
                @if (isset($mapApply))
                    <livewire:customer_portal.ebps.building_registration_form :$action :$mapApply />
                @else
                    <livewire:customer_portal.ebps.building_registration_form :$action />
                @endif
            </div>
        </div>
    </div>
