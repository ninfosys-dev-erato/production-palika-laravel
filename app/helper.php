...
if (!function_exists('getFileTypeIcon')) {
    ...
    {
        $amount = (float) $amount;
        $amount = replaceNumbers(number_format($amount, 2, '.', ','),true);
        return $amount;
    }

function amountToNepaliWords($number, $isRecursive = false) {
    // Nepali numbers up to 99
    $units = [
        0 => 'शून्य',
        1 => 'एक',
        2 => 'दुई',
    ...
    if (strpos((string)$number, ".") !== false) {
        ...
        $rupees = (int)$parts[0];
        $paisa = (int)substr(str_pad($parts[1], 2, "0"), 0, 2); // only 2 digits
        $words = '';
        if ($rupees > 0) {
            $words .= amountToNepaliWords($rupees, true) . " रूपैयाँ";
        }
        if ($paisa > 0) {
            $words .= " " . amountToNepaliWords($paisa, true) . " पैसा";
        }
        return trim($words);
    }
    ...
    $number = (int)$number;
    if ($number == 0) {
        return $units[0] . ($isRecursive ? "" : " रूपैयाँ");
    }
    ...
    foreach ($places as $value => $label) {
        if ($number >= $value) {
            ...
            $result .= amountToNepaliWords($count, true) . " " . $label . " ";
            $number %= $value;
        }
    }
    ...
    if ($number > 0) {
        if ($number <= 99) {
            ...
            $result .= amountToNepaliWords($number, true);
        }
    }

    // Only add "रूपैयाँ" if this is the top-level call
    return trim($result) . ($isRecursive ? "" : " रूपैयाँ");
}
